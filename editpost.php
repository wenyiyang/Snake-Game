<?php 
  ...
  $query = "SELECT Post_ID, Post_Title, Post_Con, Post_Des FROM Post WHERE Post_ID = '".$_GET['postId']."'";
  ...
  if(isset($_POST["submit"])) {
    ...
    if(!isset($error)){
      $query = "UPDATE Post SET Post_Title = '".$postTitle."', Post_Des = '".$postDesc."', Post_Con = '".$postCont."'WHERE Post_ID = '".$_GET['postId']."'";
      $result = mysqli_query($link,$query);
      if($result) {
        $id = $_GET['id'];
        header("Location: home.php?action=updated&id=$id");
        exit();
      }
    } 
  }
